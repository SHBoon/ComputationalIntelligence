...
#   THRESHOLDS_MF_CLASS, THRESHOLDS_CNN_CLASS, THRESHOLDS_CNN,
#   THRESHOLDS_PEAK_CLASS, SpikeCNN, detect_peaks, add_noise_for_dataset
# -------------------------------------------------------------------
...
def build_d1_training_set(data_clean, indexes, classes,
   ...
   augmentation (noise, scaling, small shift).
   """
   N = len(data_clean)
   train_features = []
   train_labels = []
...
def run_full_pipeline_on_data(data, ds_label, model,
                           GT_idx, GT_cls,
                           templates, amp_norm,
                           Fs=25000,
                           window_pre=100,
                           window_post=100):
   """
   Run your full MF + CNN pipeline on 'data'
   (D1 recording with a chosen noise profile),
   using thresholds for 'ds_label' (D1..D6), then evaluate against
   D1 ground truth (GT_idx, GT_cls).
   """
   N = len(data)
   num_classes = 5
   ...
   # -----------------------------
   # 4) Extract spike windows & CNN features
   # -----------------------------
   spike_windows = []
   valid_spike_times = []
   ...
   # -----------------------------
   # 5) CNN inference + confidence gating
   # -----------------------------
   model.eval()
   with torch.no_grad():
   ...
   print(f"\n[{ds_label}] Detection Performance (vs clean D1 GT):")
   print("  TP:", TP)
   print("  FP:", FP)
   print("  FN:", FN)
...
if __name__ == "__main__":
   # --------------------------------------------------------------
   # 0) Load clean D1 + ground truth
   # --------------------------------------------------------------
   d1_path = 'Coursework/Coursework C/Coursework_C_Datasets/D1.mat'
   d1_mat = spio.loadmat(d1_path, squeeze_me=True)

   data_clean = d1_mat['d']
   GT_idx = np.asarray(d1_mat['Index']).ravel()
   GT_cls = np.asarray(d1_mat['Class']).ravel()
   ...
   # --------------------------------------------------------------
   # 3) Sweep D1 through the coursework noise profiles
   #    using your existing add_noise_for_dataset mapping
   # --------------------------------------------------------------
   noise_profiles = ["D1", "D2", "D3", "D4", "D5", "D6"]

   for ds_label in noise_profiles:
      print("\n" + "=" * 70)
      print(f"Evaluating D1 with noise profile: {ds_label}")
      print("=" * 70)

      data_noisy = add_noise_for_dataset(
         data_clean.copy(), ds_label, Fs
      )
